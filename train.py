...
def run(args):
    ...
    for epoch in range(epochs):
        ...
        # 保存最佳模型（根據驗證 loss）
        current_val_loss = float(trainer.val_loss.result())
        if epoch == 0 or current_val_loss < best_val_loss:
        ...
        # 定期保存（每 10 個 epoch 或最後一個 epoch）
        if (epoch+1) % 10 == 0 or (epoch+1) == epochs:
            checkpoint_path_epoch = os.path.join(path_experiment, f'checkpoint_epoch_{epoch+1}')
            print(f"💾 保存 Epoch {epoch+1} checkpoint")
            try:
                unet_model.save_weights(checkpoint_path_epoch)
            except Exception as e:
                print(f"⚠️  保存失敗: {e}")
