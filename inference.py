...
def run(args):
    ...
    # 可調式降噪強度：混合原始與降噪音訊
    denoising_strength = float(args.inference.get('denoising_strength', 1.0))
    print(f"\n降噪強度: {denoising_strength} (0.0=無降噪, 1.0=完全降噪)")
    
    # 混合公式：output = strength * denoised + (1 - strength) * original
    final_output = denoising_strength * denoised_data + (1.0 - denoising_strength) * data
    
    # 根據降噪強度生成不同檔名，避免覆蓋
    basename=os.path.splitext(audio)[0]
    strength_suffix = f"_s{int(denoising_strength*10)}" if denoising_strength != 1.0 else ""
    
    wav_noisy_name=basename+"_noisy_input"+".wav"
    sf.write(wav_noisy_name, data, 44100)
    wav_output_name=basename+f"_denoised{strength_suffix}"+".wav"
    sf.write(wav_output_name, final_output, 44100)
    wav_output_name=basename+f"_residual{strength_suffix}"+".wav"
    sf.write(wav_output_name, residual_noise, 44100)
    
    print(f"\n已儲存:")
    print(f"  原始輸入: {wav_noisy_name}")
    print(f"  降噪輸出: {basename}_denoised{strength_suffix}.wav (強度={denoising_strength})")
    print(f"  殘差噪音: {basename}_residual{strength_suffix}.wav")
